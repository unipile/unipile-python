...
class PublishInPromotedMode(BaseModel):
    """
    ...
    mode: StrictStr
    budget: PublishInPromotedModeBudget
    __properties: ClassVar[List[str]] = ["set_hiring_frame", "bypass_email_verification", "mode", "budget"]

    @field_validator('mode')
    def mode_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PROMOTED', 'PROMOTED_PLUS']):
            raise ValueError("must be one of enum values ('PROMOTED', 'PROMOTED_PLUS')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "set_hiring_frame": obj.get("set_hiring_frame") if obj.get("set_hiring_frame") is not None else True,
            "bypass_email_verification": obj.get("bypass_email_verification") if obj.get("bypass_email_verification") is not None else False,
            "mode": obj.get("mode"),
            "budget": PublishInPromotedModeBudget.from_dict(obj["budget"]) if obj.get("budget") is not None else None
        })
        return _obj

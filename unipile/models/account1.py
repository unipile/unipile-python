...
class Account1(BaseModel):
    ...
    status: StrictStr = Field(description="The current status of the account.     - `running` is Operational.     - `errored` is Service Interuption.     - `disconnected` is Require Authentication..     - `paused` is Paused.     Learn more about [Status & Lifecycle](https://developer.unipile.com/v2.0/docs/status-lifecycle).")

...
class CreateNewExperience(BaseModel):
    ...
    employment_type: Optional[StrictStr] = Field(default=None, description="A parameter ID. Use <a href=\"https://developer.unipile.com/v2.0/reference/getclassicsearchparameters\">List Search Parameters</a> with `EMPLOYMENT_TYPE` type to find out the possible values. Employment type of the experience.")
    ...
    skills: Optional[List[CreateNewExperienceSkillsInner]] = Field(default=None, description="Skills to associate with this experience. Use this field only inside `experience`; it is independent from the profile-level and education `skills` fields. These skills also appear in the profile Skills section.")

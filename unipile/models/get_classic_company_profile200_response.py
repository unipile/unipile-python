        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.viewer_roles:
            for _item_viewer_roles in self.viewer_roles:
                _items.append(_item_viewer_roles.to_dict() if _item_viewer_roles is not None else None)
            _dict['viewer_roles'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in locations (list)
        _items = []
        if self.locations:
            for _item_locations in self.locations:
                _items.append(_item_locations.to_dict() if _item_locations is not None else None)

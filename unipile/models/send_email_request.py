...
class SendEmailRequest(BaseModel):
    ...
    SendEmailRequest
    """ # noqa: E501
    html: Optional[StrictStr] = Field(default=None, description="The HTML content of the email. Required for tracking.")
    plain_text: Optional[StrictStr] = Field(default=None, description="The plain text content of the email.")
    ...
    reply_to: Optional[List[GetEmailsList200ResponseDataInnerFromInner]] = Field(default=None, description="List of attendees to set in the standard `Reply-To` header. Recipients answering the email will reply to these addresses instead of `from`. To reply inside an existing conversation, use `reply_to_message_id` instead.")
    reply_to_message_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="The ID of the email message that you're replying to. For Gmail and Outlook, this is the provider ID for the email message that you're replying to. For IMAP Send, this is the RFC822 Message-ID header of the email message that you're replying to.")
    attachments: Optional[List[SendEmailRequestAttachmentsInner]] = Field(default=None, description="The list of file attachments to the email.")
    custom_headers: Optional[List[SendEmailRequestCustomHeadersInner]] = Field(default=None, description="An array of custom headers to add to the email. Each header overrides any existing header with the same name (case-insensitive), including headers set by other fields such as `to`, `cc`, `bcc`, `from`, `subject`, or `reply_to`. A `Reply-To` custom header therefore takes precedence over the `reply_to` field and its value is used verbatim.")
    tracking_options: Optional[SendEmailRequestTrackingOptions] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["html", "plain_text", "subject", "from", "to", "cc", "bcc", "reply_to", "reply_to_message_id", "attachments", "custom_headers", "tracking_options"]
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        if self.custom_headers:
            for _item_custom_headers in self.custom_headers:
                _items.append(_item_custom_headers.to_dict() if _item_custom_headers is not None else None)
            _dict['custom_headers'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "custom_headers": [SendEmailRequestCustomHeadersInner.from_dict(_item) for _item in obj["custom_headers"]] if obj.get("custom_headers") is not None else None,

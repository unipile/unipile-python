...
class LeadListDataInner(BaseModel):
    """
    LeadListDataInner
    """ # noqa: E501
    ...
    industry: Optional[StrictStr] = Field(default=None, description="The industry in which the User works.")
    ...
    languages: List[LinkedIn1LanguagesInner]
    volunteering: List[LinkedIn1VolunteerExperienceInner]
    object: StrictStr
    saved_at: StrictStr = Field(description="The date on which the Lead was saved. Uses ISO 8601 UTC datetime (YYYY-MM-DDTHH:MM:SS.sssZ).")
    associated_account: Optional[LeadListDataInnerAllOfAssociatedAccount] = None
    __properties: ClassVar[List[str]] = ["id", "member_id", "display_name", "public_identifier", "profile_url", "public_picture_url", "public_picture_url_large", "relations_count", "location", "headline", "network_distance", "can_send_inmail", "shared_relations_count", "summary", "has_posted_recently", "has_viewed_your_profile", "was_hired_recently", "was_promoted_recently", "follows_your_company", "is_premium", "is_past_colleague", "is_open_profile", "websites", "addresses", "contact_info", "latest_contact", "lists_count", "notes_count", "social_handles", "industry", "education", "skills", "work_experience", "languages", "volunteering", "object", "saved_at", "associated_account"]

    @field_validator('object')
    def object_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SavedLead']):
            raise ValueError("must be one of enum values ('SavedLead')")
        return value
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.work_experience:
            for _item_work_experience in self.work_experience:
                _items.append(_item_work_experience.to_dict() if _item_work_experience is not None else None)
        ...
        if self.volunteering:
            for _item_volunteering in self.volunteering:
                _items.append(_item_volunteering.to_dict() if _item_volunteering is not None else None)
            _dict['volunteering'] = _items
        # override the default output from pydantic by calling `to_dict()` of associated_account
        if self.associated_account:
            _dict['associated_account'] = self.associated_account.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "volunteering": [LinkedIn1VolunteerExperienceInner.from_dict(_item) for _item in obj["volunteering"]] if obj.get("volunteering") is not None else None,
            "object": obj.get("object"),
            "saved_at": obj.get("saved_at"),
            "associated_account": LeadListDataInnerAllOfAssociatedAccount.from_dict(obj["associated_account"]) if obj.get("associated_account") is not None else None
        })
        return _obj
